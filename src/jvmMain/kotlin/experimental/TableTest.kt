...
fun DefaultDataTableSorting(
    ...
    val column = remember { mutableStateOf(initialColumn) }
    val ascending = remember { mutableStateOf(initialAscending) }
...
fun DataTable(
    ...
    cellSpacing: PaddingValues = CellSpacing,
    ...
    val table = @Composable {
        Table(
            ...
            if (header != null) {
                tableRow {
                    if (showCheckboxes) {
                        Box(modifier = Modifier.height(headerRowHeight).padding(cellSpacing)) {
            ...
            visibleRows.forEach { row ->
                tableRow {
                    if (showCheckboxes) {
                        Box(modifier = Modifier.height(dataRowHeight).padding(cellSpacing)) {
...
private val CellSpacing = PaddingValues(start = 16.dp, end = 16.dp)
